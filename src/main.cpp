...
void	mock_server()
{
	...
	{ if (sock_server == -1){ std::cout << "Error creating socket" << std::endl; return ; }}
	...
	{ if (ret == -1){ std::cout << "Error binding socket" << std::endl; return ; }}

	ret = listen(sock_server, 10);
	{ if (ret == -1){ std::cout << "Error listening socket" << std::endl; return ; }}

	while (true)
	{

		int sock_client = accept(sock_server, NULL, NULL);
		{ if (sock_client == -1){ std::cout << "Error accepting socket" << std::endl; return ; }}
